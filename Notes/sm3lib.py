...
def Message_extension(a):
    ...
    for j in range(16, 68):
        temp = (
            P1(W1[j - 16] ^ W1[j - 9] ^ cycle_shift_left(W1[j - 3], 15))
            ^ cycle_shift_left(W1[j - 13], 7)
            ^ W1[j - 6]
        )
        W1.append(temp)
...
def SM3(plaintext):
    ...
    m = (
        ...
        + "8"
        + "0" * int((k + 1) / 4 - 1)
        + zero_fill(str(hex(len(plaintext) * 4))[2:], 16)
    )
...
def SM3_len_ex_ak(num_block, IV, plaintext):
    ...
    m = (
        ...
        + "8"
        + "0" * int((k + 1) / 4 - 1)
        + zero_fill(str(hex(len(plaintext) * 4 + num_block * 512))[2:], 16)
    )
    block_len = int((len(plaintext) * 4 + k + 65) / 512)
    for i in range(0, block_len):
        B.append(m[128 * i : 128 * i + 128])
